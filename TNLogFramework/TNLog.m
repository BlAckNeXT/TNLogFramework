...
+ (void)mwapLogWithParDictionary:(NSDictionary *)dictionary
{
    ...
    NSString *operation=@"";
    if (dictionary[@"ci"]) {
        feedId = [NSString stringWithFormat:@"%@",dictionary[@"ci"]];
    }
    if (dictionary[@"ck"]) {
        cookie = [NSString stringWithFormat:@"%@",dictionary[@"ck"]];
    }
    if (dictionary[@"ai"]) {
        appPlugId = [NSString stringWithFormat:@"%@",dictionary[@"ai"]];
    }
    if (dictionary[@"av"]) {
        appPlugVersion = [NSString stringWithFormat:@"%@",dictionary[@"av"]];
    }
    if (dictionary[@"rf"]) {
        referrer = [NSString stringWithFormat:@"%@",dictionary[@"rf"]];
    }
    if (dictionary[@"url"]) {
        url = [NSString stringWithFormat:@"%@",dictionary[@"url"]];
    }
    if (dictionary[@"ti"]) {
        title = [NSString stringWithFormat:@"%@",dictionary[@"ti"]];
    }
    if (dictionary[@"ot"]) {
        operation = [NSString stringWithFormat:@"%@",dictionary[@"ot"]];
    }
    
    NSDictionary *parDic = @{@"sign":sign,
                             @"t":[NSString stringWithFormat:@"%.0f",t],
                             @"appKey":kAppKey,
                             @"data":@{@"toonData":@{@"sf":[TNLog getStartFlag],
                                                     @"dn":[TNLog getDeviceNumber],
                                                     @"ui":[TNLog getUserId],
                                                     @"dv":@"2"},
                                       @"bizData":@{@"ci":feedId,
...
+(void)deviceInfoWithOperationType:(OPERATION_TYPE)operationType
{
    ...
    NSString *parStr = [self dictionaryToJson:parDic];
    NSDictionary *resDic = @{@"key":parStr};
    NSLog(@"基本设备信息日志上传参数 : %@      == url:%@",resDic,kSaveDeviceInfo);
    AFHTTPRequestOperationManager *manager = [TNLog AFManagerInit];
